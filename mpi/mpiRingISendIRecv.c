...
int main(int argc,char** argv)
{
   ...
   /* make the random numbers repeatable: */
   srand(taskid);
   ...
   if(taskid==0){
     for(itask=0;itask<ntasks;itask++){
       ...
       printf("Task %2d : Sum of vector sent to %2d = %e\n",
   ...
   /* simpler: */
   {
        int taskright = (taskid + 1) % ntasks ;
        int taskleft  = (taskid - 1 + ntasks) % ntasks ;
        ierr=MPI_Isend(sendbuff,buffsize,MPI_DOUBLE,
	           taskright, 0,MPI_COMM_WORLD,&send_request);   
        ierr=MPI_Irecv(recvbuff,buffsize,MPI_DOUBLE,
	           taskleft,  MPI_ANY_TAG,MPI_COMM_WORLD,&recv_request);
        recvtime = MPI_Wtime() - inittime ;
    }
   ...
   if(taskid==0){
     ...
     for(itask=0;itask<ntasks;itask++){
       printf("Task %2d : Sum of received vector= %e : Time=%f seconds\n",
